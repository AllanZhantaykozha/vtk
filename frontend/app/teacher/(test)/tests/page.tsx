...
export default function TestsPage() {
    ...
    const fetchData = async () => {
      try {
        ...
        const [testsRes, subjectsRes] = await Promise.all([
          fetch("http://localhost:4000/teacher/tests", {
  ...
  const topics = useMemo(
    () => [...new Set(tests.map((test) => test.title))],
    [tests]
  );
  ...
  const updateFilters = useCallback(
    (newFilters: Partial<Record<string, string>>) => {
      ...
      router.push(params.toString() ? `tests?${params}` : "tests", {
        scroll: false,
      });
    },
    [router, searchParams]
  ...
  const filteredTests = useMemo(() => {
    return tests.filter((test) => {
      ...
      const matchesSubject =
        selectedSubject === "all" || test.subject.name === selectedSubject;
      const matchesTopic =
        selectedTopic === "all" || test.title === selectedTopic;
      const testDate = new Date(test.uploadDate);
      const matchesStartDate = startDate
        ? testDate >= new Date(startDate)
        : true;
      const matchesEndDate = endDate ? testDate <= new Date(endDate) : true;
  ...
  }, [tests, searchTerm, selectedSubject, selectedTopic, startDate, endDate]);
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="flex justify-between items-center mb-8">
          <h1 className="text-3xl font-bold text-gray-800">
            {selectedSubject === "all"
              ? `Список тестов`
              : `Список тестов по предмету ${selectedSubject}`}
          </h1>
            ...
            filteredTests.map((test) => (
              ...
              >
                ...
                <div className="grid grid-cols-2 w-full gap-2">
                  <Button onClick={() => router.push(`tests/${test.id}`)}>
                    Открыть
                  </Button>
                  <Button onClick={() => router.push(`updateTest/${test.id}`)}>
                    Изменить
                  </Button>
                </div>
              </div>
